...
for index in range(len(list1)):
	...
	while True:
		im_name='uploads/'+list1[i]
			...
			cursor2.execute("select name from users where id=%s",(nbr_predicted))
			data2=cursor2.fetchall()
			for row in data2:
				pred_name=row[0]
				print(pred_name)
        		
        		cv2.cv.PutText(cv2.cv.fromarray(im),"name="+(pred_name)+"//id="+str(nbr_predicted), (x-50,y),font, 900) #Draw the text
    		#cv2.imshow('im',im)    
